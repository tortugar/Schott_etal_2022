...
def get_lsr_surround(ppath, recordings, istate, win, signal_type, recalc_highres=False,
    ...
    for rec in recordings:
        ...
        # SP calculated using EEG2
        if ('SP2' in signal_type) and (signal_type != 'SP2_CALC'):
            SP, f, t, sp_dt, sp_nbin, _ = AS.highres_spectrogram(ppath, rec, nsr_seg, perc_overlap, recalc_highres, 'EEG2')
            #sp_nbin = len(EEG) / SP.shape[1]
            sp_win1 = int(round(iwin1/sp_nbin))
            sp_win2 = int(round(iwin2/sp_nbin))
        ...
        # SP calculated using EEG
            ...
            SP, f, t, sp_dt, sp_nbin, _ = AS.highres_spectrogram(ppath, rec, nsr_seg, perc_overlap, recalc_highres, 'EEG')
            sp_win1 = int(round(iwin1/sp_nbin))
            sp_win2 = int(round(iwin2/sp_nbin))
            lsr_sp_win1 = int(round(lsr_iwin1/sp_nbin))
            lsr_sp_win2 = int(round(lsr_iwin2/sp_nbin))
    ...
    END = time.perf_counter()
    print(f'COMPUTING TIME --> {END-START:0.2f} seconds ({len(recordings)} recordings, {len(istate)} brainstates, signal type = {signal_type})')
    return lsr_pwaves, spon_pwaves, success_lsr, fail_lsr, null_pts, data.shape
...
def get_band_pwr(sp_dict, f, bands, mouse_avg='mouse', pnorm=0, psmooth=0, sf=0, fmax=0):
    ...
    mouse_avg - if 'mouse'  - mx of mice x time bins for each power band
                if 'trials' - mx of trials x time bins for each power band
...
def detect_pwaves(ppath, recordings, channel, thres, thres_init=5, dup_win=40,
    ...
    ma_thr=20; ma_state=6; flatten_tnrem=3
...
def get_lsr_pwaves(p_idx, lsr, post_stim=0.1, sr=1000., lat_th=0):
    """
    ...
    post_stim - P-waves within $post_stim s of laser onset are "laser-triggered"
                ...
                Laser pulses followed by 1+ P-waves within $post_stim s are "successful"
                  (all others are "failed")
    sr - sampling rate (Hz)
    lat_th - optional threshold for latency of a triggered P-wave from laser pulse onset (ms)
    ...
    if lat_thr != 0:
        for li in lsr_p_idx:
            ...
            if (lat_thr > 0) and lat <= lat_th:  # remove low latencies
                lat_elim.append(li)
            elif (lat_thr < 0) and lat >= lat_th:  # remove high latencies
                lat_elim.append(li)
        for sl in success_lsr:
            ptrig = lsr_p_idx[np.where(lsr_p_idx > sl)[0][0]]
            lat = round((ptrig - sl)/1000*sr, 4)
            if (lat_thr > 0) and lat <= lat_th:
                lat_elim.append(sl)
            elif (lat_thr < 0) and lat >= lat_th:
                lat_elim.append(sl)
    lsr_p_idx = [li for li in lsr_p_idx if li not in lat_lo]
    success_lsr = [sl for sl in success_lsr if sl not in lat_lo]

    return lsr_p_idx, spon_p_idx, success_lsr, fail_lsr

def get_p_iso(p_idx, sr, win=0.8):
    """
...
def get_lsr_cluster(ppath, recordings, istate, post_stim=0.1, p_iso=1, pcluster=0.5, 
                    clus_event='waves', ma_thr=20, ma_state=3, flatten_tnrem=False):
    """
...
def get_lsr_stats(ppath, recordings, istate, post_stim=0.1, p_iso=0, pcluster=0, 
    ...
    cols = ['mouse', 'rec', 'state', 's', 's_seq', 'perc_seq', 'amp', 'amp2', 'phase', 'latency', 'halfwidth']
...
def inter_pwave_intervals(ppath, recordings, istate, bins=50, prange=(0,20), 
    ...
    """
    states = {0:'all', 1:'REM', 2:'Wake', 3:'NREM', 4:'tNREM', 5:'failed-tNREM', 6:'Microarousals'}
    intervals = []
    for rec in recordings:
        print('Getting P-waves for ' + rec + ' ...')
...
def pwave_overview(ppath, recordings, tstart=0, tend=-1, ma_thr=20, ma_state=3, flatten_tnrem=False):
    """
    ...
    for rec in recordings:
        ...
        if tend == -1:
            ...
            M = M[istart : int(round(tend/dt)) + 1 ]
            p_freq_dn = p_freq_dn[istart : int(round(tend/dt)) + 1 ]
        
        # create plot
        fig, (ax1, ax2) = plt.subplots(figsize=(10,3), nrows=2, ncols=1, sharex = True, gridspec_kw={'height_ratios':[1,3]})
        t = np.arange(istart, istart + len(M))*dt
...
def dff_timecourse(ppath, recordings, istate, dff_win=[-10,10], plotMode='0',
                   ...
                   vm=[], psmooth=0, ds=0, sf=0, mouse_avg='mouse', base_int=10,
                   baseline_start=0, baseline_end=-1, ma_thr=20, ma_state=3, 
                   flatten_tnrem=False, jitter=0, ylim=[], ylim2=[], print_stats=True):
    """
    ...
    ds - downsample DF/F timecourse & heatmap along time dimension (for a manageable # of plot points)
    ...
    for rec in recordings:
        ...
        for i in state_p_idx:
            if i>=np.abs(iwin1) and i+iwin2<len(dff):
    ...
    mx_pwave = mx2d(pwave_trig, 'trials', d1_size=(iwin1+iwin2))[0]        # trials x time bins
    mx_pwave_z1 = mx2d(pwave_trig_z, 'trials', d1_size=(iwin1+iwin2))[0]   # trials x time bins z-scored by recording
    ...
    for i,z in enumerate(pzscore):
        if mouse_avg in ['trial', 'trials'] or i==1:  # heatmap always uses single trials
            if z == 0:
    ...
    if 't' in plotMode:  # timecourse of DF/F signal
        ...
        if ds:
            graph_mx[0] = AS.downsample_mx(graph_mx[0], ds, axis='x')
    ...
    if 'h' in plotMode:  # heatmap of individual P-wave trials
        # smooth heatmap
        if psmooth:
            graph_mx[1] = AS.convolve_data(graph_mx[1], psmooth, axis='xy')
        # downsample along time dimension
        if ds:
            graph_mx[1] = AS.downsample_mx(graph_mx[1], ds, axis='x')
    ...
    if '0' in plotMode:  # bar graph of DF/F activity preceding vs following P-wave
...
def spectralfield_highres(ppath, name, pre, post, istate=[1], theta=[1,10,100,1000,10000], pnorm=1,
                          ...
                          ma_thr=20, ma_state=3, flatten_tnrem=False, pzscore=False, pplot=True):
    """
    ...
    M = AS.adjust_brainstate(M, dt=2.5, ma_thr=ma_thr, ma_state=ma_state, flatten_tnrem=flatten_tnrem)
...
def spectralfield_highres_mice(ppath, recordings, pre, post, istate=[1], theta=[1,10,100,1000,10000],
                               ...
                               ma_thr=20, ma_state=3, flatten_tnrem=False, pzscore=False, vm=[]):
    """
    ...
    for rec in recordings:
        idf = re.split('_', rec)[0]
        k,t,f = spectralfield_highres(ppath, rec, pre=pre, post=post, istate=istate, theta=theta,
                                      pnorm=pnorm, psmooth=psmooth, fmax=fmax, recalc_highres=recalc_highres,
                                      nsr_seg=nsr_seg, perc_overlap=perc_overlap, ma_thr=ma_thr, ma_state=ma_state,
                                      flatten_tnrem=flatten_tnrem, pzscore=pzscore, pplot=False)
...
def state_freq(ppath, recordings, istate, plotMode='0', tstart=0, tend=-1,
               ma_thr=20, ma_state=3, flatten_tnrem=False, wf_win=[0.5,0.5],
               p_iso=0, pcluster=0, clus_event='waves', noise_state=2, 
               pplot=True, print_stats=True, ylim1=[], ylim2=[]):
    """
    ...
    x_labels - list of brain states, corresponding to columns/layers in freq_mx & waveform_mx
    ...
    states = {0:'sleep', 1:'REM', 2:'Wake', 3:'NREM', 4:'tNREM', 5:'failed-tNREM', 6:'Microarousals'}
    ...
    waveform_mouse = {}
    for m in mice:
        freq_mouse[m] = {s:[] for s in istate}
        waveform_mouse[m] = {s:[] for s in istate}
    ...
    for rec in recordings:
        ...
        M = AS.adjust_brainstate(M, dt=dt, ma_thr=ma_thr, ma_state=ma_state, 
                                 flatten_tnrem=flatten_tnrem, noise_state=noise_state)
        ...
        # downsample P-waves, get frequency as P-waves /s
        (p_idx_dn, p_idx_dn_exact, p_count_dn, p_freq_dn) = downsample_pwaves(LFP, idx, sr=sr, 
                                                                              nbin=nbin, rec_bins=len(M))
        p_freq_dn *= dt
        ...
        if tend==-1:
            ...
            iend_lfp = int(np.round(tend*sr))
        istart = int(np.round((1.0*tstart) / dt))
        istart_lfp = int(np.round(tstart*sr))
        M = M[istart:iend]
        p_freq_dn = p_freq_dn[istart:iend]
        LFP = LFP[istart_lfp : int(iend_lfp)]
        # get indices of time window for LFP waveform
        iwin1, iwin2 = get_iwins(wf_win, sr, inclusive=False)
        
        # collect P-wave frequency and waveforms for each brain state
        for s in istate:
            if s == 0:
                state_idx = np.arange(0, len(M))
            else:
                # get brain state sequences and indices
                sseq = sleepy.get_sequences(np.where(M==s)[0])
                state_idx = list(chain.from_iterable(sseq))
            
            # if no instances of brain state
            if len(state_idx) == 0:
                freq_mouse[idf][s].append(np.nan)
                emp = np.empty((iwin1+iwin2,))
                emp[:] = np.nan
                # collect NaN array instead of waveforms, prevent plotting errors
                waveform_mouse[idf][s].append( [emp] )
                continue

            # get mean P-wave frequency in state
            if state_idx[-1] == len(p_freq_dn):
               freq_mouse[idf][s].append( p_freq_dn[state_idx[0:-1]] )
            else:
                freq_mouse[idf][s].append( p_freq_dn[state_idx] )
            
            # find all P-waves in state, collect waveforms
            state_p_idx = [i for i in idx if int(i/nbin) in state_idx]
            state_waveforms = []
            for spi in state_p_idx:
                if spi>=iwin1 and spi+iwin2 < len(LFP):
                    state_waveforms.append(LFP[spi-iwin1:spi+iwin2])
            
            # if no waveforms in this state, collect NaN array
            if len(state_waveforms) == 0:
                emp = np.empty((iwin1+iwin2,))
                emp[:] = np.nan
                waveform_mouse[idf][s].append( [emp] )
            # otherwise, collect average state waveform
            else:
                waveform_mouse[idf][s].append( state_waveforms )
                state_counts[s] += len(state_waveforms)
    
    # create matrices for P-wave frequencies (mice x state) and waveforms (mice x time bins x state)
    freq_mx = np.zeros((len(mice), len(state_counts)))
    waveform_mx = np.zeros((len(mice), iwin1+iwin2, len(state_counts)))
    for midx, mouse in enumerate(mice):
        for sidx, state in enumerate(istate):
            # collect avg P-wave frequency and LFP waveform in brain state 
            f = [np.nanmean(r_f) for r_f in freq_mouse[mouse][state]]
            w = [np.nanmean(r_w, axis=0) for r_w in waveform_mouse[mouse][state]]
            freq_mx[midx, sidx] = np.nanmean(f)
            waveform_mx[midx, :, sidx] = np.nanmean(w, axis=0)
    ...
    for k in state_counts:
        if isinstance(k, int):
            print(states[k], ':', state_counts[k], 'P-waves')
            x_labels.append(states[k])
        elif isinstance(k, str):
            print(k, ':', state_counts[k], 'P-waves')
            x_labels.append(k)
    
    # plot P-wave frequency and avg waveform in each brain state
    if pplot:
        plot_state_freq(x_labels, mice, freq_mx, waveform_mx, plotMode=plotMode, 
                        ylim2=ylim2, group_colors='states', group_labels=[], legend='all')
    
    # create dataframe with P-wave frequencies
    df = pd.DataFrame({'Mouse' : np.tile(mice, len(x_labels)),
                      'State' : np.repeat(x_labels, len(mice)),
                      'Freq' : np.reshape(freq_mx,-1,order='F')})
    
    # stats
    if print_stats and len(istate) > 2:
        res_anova = AnovaRM(data=df, depvar='Freq', subject='Mouse', within=['State']).fit()
        mc = MultiComparison(df['Freq'], df['State']).allpairtest(stats.ttest_rel, method='bonf')
        #print(res_anova); print(''); print(mc[0])
        print(res_anova); print('p = ' + str(float(res_anova.anova_table['Pr > F']))); print(''); print(mc[0])
    
    return mice, x_labels, freq_mx, waveform_mx
...
def activity_transitions(ppath, recordings, transitions, pre, post, si_threshold, sj_threshold, mode='pwaves',
                         tstart=0, tend=-1, ma_thr=20, ma_state=3, flatten_tnrem=False, fmax=30, pnorm=1,
    ...
    if pplot:
        ...
        for (si,sj) in transitions:
            ...
            if nmice == 1:
                ...
                ax.fill_between(np.linspace(-pre, post, len(tmp)), tmp - sem, tmp + sem, color=(0, 0, 0), alpha=0.4, edgecolor=None)
            sleepy.box_off(ax)
...
def stateseq(ppath, recordings, sequence, nstates, state_thres, sign=['>','>','>'], 
             mode='pwaves', tstart=0, tend=-1, ma_thr=20, ma_state=3, flatten_tnrem=False,
             p_iso=0, pcluster=0, clus_event='waves', fmax=30, pnorm=1, psmooth=0, vm=[],
             mouse_avg='mouse', base_int=10, base_start=0, base_end=-1, xlim=[], ylim=[],
             pzscore=False, pplot=True, print_stats=True):
    """
    ...
    for rec in recordings:
        ...
        for init_seq in seqi:
            ...
            if state_thres[0][0] < len(init_seq)*dt <= state_thres[0][1]:
                ...
                for i, state in enumerate(sequence[1:]):
                    ...
                    if state_thres[i+1][0] < len(next_seq)*dt <= state_thres[i+1][1]:
                        si = next_seq[-1]
    ...
    for mouse in mice:
        ...
        [seq_pwave['trials'].append(trial) for trial in pwave_mouse[mouse]]
        [seq_spe['trials'].append(trial) for trial in spe_mouse[mouse]]
            
    # create matrix for signal (mouse/trial x time bins) and SPs (freq x time bins x mouse/trial)
    i = 0
    if mouse_avg == 'mouse':
        ...
        mx_pwave = np.zeros((len(seq_pwave['trials']), sum(nstates)))
        mx_spe = np.zeros((len(seq_spe['trials']), len(ifreq), sum(nstates)))
        for pt, st in zip(seq_pwave['trials'], seq_spe['trials']):
            mx_pwave[i,:] = pt
            mx_spe[i,:,:] = st
            i += 1
    ...
    if pplot:
        ...
        if nmice > 1:
            ...
            ax_pwave.fill_between(x, tmp - sem, tmp + sem, color=(0, 0, 0), alpha=0.3, edgecolor=None)
        # set axis labels
...
def plot_activity_transitions(mx_list, mice=[], sem=True, plot_id=[], group_labels=[], xlim=[], xlabel='', ylabel='', title=''):
    """
    Plot timecourses of signals (absolute or normalized time) during brain state transitions, for multiple groups of mice
    @Params
    ...
    plot_id - optional list specifying a color (for mouse group) or linestyle (for individual mice) to plot timecourses
    group_labels - optional list of legend labels for each group 
    xlim - if plotting absolute time transitions between 2 states --> 2-element list if [-pre, post] seconds before/after transition
           if plotting time-normalized transitions across N states --> N-element list of the no. of bins in each state
    ...
    for i, mx in enumerate(mx_list):
        ...
        if sem:
            ...
            ax.fill_between(t, mx_mean - mx_sem, mx_mean + mx_sem, color=plot_id[i], alpha=0.4, edgecolor=None)
        # plot individual mice
        else:
            if len(mice) > 0:
                ms_names = mice[i]
                for row in range(len(mx)):
                    ax.plot(t, mx[row, :], color=mcs[ms_names[row].split('_')[0]], linestyle=plot_id[i], label=ms_names[row])
            else:
                for row in range(len(mx)):
                    ax.plot(t, mx[row, :], color=mcs[row], linestyle=plot_id[i])
            # set legend for mouse group linestyles
            ax.plot(t[0], min(mx_mean), linestyle=plot_id[i], color='black', label=group_labels[i])
    # set axis labels and limits
    ...
    if sem:
        ...
        if len(mice) > 0:
            AS.legend_mice(ax, list(chain.from_iterable(mice)))
            AS.legend_lines(ax, skip=list(chain.from_iterable(mice)), loc='upper center')
        else:
            AS.legend_lines(ax)
    plt.show()

def sleep_timecourse(ppath, recordings, istate, tbin, n, stats, plotMode='0', tstart=0, tend=-1, 
                     ma_thr=20, ma_state=3, flatten_tnrem=False, pzscore=False, pplot=True, ylim=[]):
    """
    ...
    stats - type of data to plot for each time bin
            ...
            'transition probability' - percent of transition states followed by REM sleep
    ...
    if stats in ['perc', 'freq', 'dur', 'time', 'total time', 'transition probability']:
        MODE = 'STATE'
        ...
        print('')
        sys.exit()
    if stats == 'transition probability': istate = [1]
    ...
    if MODE =='STATE':
        mouse_data = {'perc':{}, 'freq':{}, 'dur':{}, 'time':{}, 'total time':{}, 'transition probability':{}}
    if MODE == 'PWAVES':
        mouse_data = {'pwave freq':{}, 'pwave amp':{}}
    if MODE == 'DFF':
        mouse_data = {'dff':{}}
    
    # create sub-dictionary for each mouse
    for md in mouse_data:
        # create sub-dict key for each state
        for m in mice:
            mouse_data[md][m] = {state:[] for state in istate}
    ...
    for rec in recordings:
        ...
        for s in istate:
            if MODE == 'STATE':
                # collect avg % time, freq, duration, and seconds spent in brain state per time bin
                bin_data = {'perc':[], 'freq':[], 'dur':[], 'time':[], 'total time':[], 'transition probability':[]}
            if MODE == 'PWAVES':
                # collect avg P-wave freq and amplitude per time bin
                bin_data = {'pwave freq':[], 'pwave amp':[]}
            if MODE == 'DFF':
                # collect avg DF/F signal per time bin
                bin_data = {'dff':[]}
            ...
            for i in range(n):
                ...
                if MODE =='STATE':
                    ...
                    bin_data['perc'].append( len(sidx) / (1.0*len(M_cut)) )
                    
                    # get state frequency
                    if sseq[0].size > 0:
                        bin_data['freq'].append(len(sseq) * (3600. / (len(M_cut)*dt)))
                    else:
                        bin_data['freq'].append(float(0))
                    
                    # get avg state duration
                    d = np.array([len(j)*dt for j in sseq])
                    if not all(d==0):
                        bin_data['dur'].append(d.mean())
                    else:
                        # if no instances of state, store NaN
                        bin_data['dur'].append(np.nan)
                    
                    # get no. of seconds spent in state
                    ssec = len(sidx)*dt
                    bin_data['time'].append(ssec)
                    
                    # get cumulative no. of seconds spent in state up to this point
                    if i==0:
                        bin_data['total time'].append(ssec)
                    else:
                        bin_data['total time'].append(ssec+bin_data['total time'][i-1])
                    ...
                    bin_data['transition probability'].append( numS/(numS+numF)*100 )
                
                if MODE == 'PWAVES':
                    # get avg P-wave frequency during state
                    p_freq_cut = p_freq_dn[istart+i*ibin:istart+(i+1)*ibin]
                    bin_data['pwave freq'].append(np.nanmean(p_freq_cut[sidx]))
                    
                    # get avg P-wave amplitude during state
                    bin_p_idx = [int(round(p*sr*dt)) for p in p_idx_dn_exact if istart+i*ibin <= p <= istart+(i+1)*ibin and M[int(round(p))] == s]
                    bin_p_amps =  np.array(([get_amp(LFP, bpi, sr) for bpi in bin_p_idx]))
                    bin_data['pwave amp'].append(np.nanmean(bin_p_amps) / 1000)  # divide by 1000 to convert Intan data from uV to mV
                if MODE == 'DFF':
                    # get avg DF/F signal during state
                    dff_cut = dff[istart+i*ibin:istart+(i+1)*ibin]
                    bin_data['dff'].append(np.nanmean(dff_cut[sidx]))
    
            # store time-binned data for brain state
            for key, data in zip(list(bin_data.keys()), list(bin_data.values())):
                mouse_data[key][idf][s].append(data)
    
    
    # create data dictionary (key=brain state, value=matrix of mice x time bins)
    TimeCourse = {s:np.zeros((len(mice), n)) for s in istate}
    # get data avg for each mouse
    for k in list(TimeCourse.keys()):
        for i,m in enumerate(mice):
            mouse_avg = np.nanmean(mouse_data[stats][m][k], axis=0)
            TimeCourse[k][i,:] = mouse_avg
    # plot bar graph for each brain state
    if pplot:
        plot_sleep_timecourse(TimeCourse, mice, tstart, tbin, stats, group_colors='states', legend='mice', plotMode=plotMode) 
    
    return mice, TimeCourse

def plot_sleep_timecourse(TimeCourse_list, mice, tstart, tbin, stats, plotMode='0',
                          compare_within=[], set_key_groups=[], group_colors=[], 
                          group_labels=[], ylims=[], titles='auto', legend=''):
    """
    ...
    if len(compare_within) == 0 or len(TimeCourse_list) > 1:
        ...
        # automatically plot all brain states
            ...
            nplots = max([len(TC.keys()) for TC in TimeCourse_list]) # get max number of keys/subplots from the largest TimeCourse dictionary
            plot_keys = []
            kidx = 0
            while kidx < nplots:
    ...
    elif stats == 'transition probability':
        YLABEL = '% transitions'
    ...
    # create figure
    plt.ion() 
    fig, axs = plt.subplots(figsize=(np.sqrt(n)*3, np.sqrt(nplots)*3), nrows=nplots, ncols=1, constrained_layout=True)
    ...
    for idx, pk in enumerate(plot_keys):
        ...
        tick_bar = np.linspace(-((width/2)*(len(matrices)-1)), (width/2)*(len(matrices)-1), len(matrices))
        ...
        for (iidx, tcidx, k, mx, mnames, gname, bcolor) in zip(range(len(matrices)), timecourse_idx, brainstate_keys, matrices, mouse_names, group_names, bar_colors):
            x = t+tick_bar[iidx]
            ...
            # plot bars
            if '0' in plotMode:
                ax.bar(x, data, width=width, yerr=sem, color=bcolor, edgecolor='black', label=gname, error_kw=dict(lw=3, capsize=0))
            else:
            ...
            for mrow, mname in enumerate(mnames):
                for col in range(n):
                    ...
                    if '1' in plotMode or '2' in plotMode:
                        if not np.isnan(Y_):
                            mh = ax.plot(X_, Y_, color=markercolor, marker='o', ms=7, markeredgewidth=1, markeredgecolor='black', label=mname, clip_on=False)[0]
                        else:
                             mh = ax.plot(X_, 0, color=markercolor, marker='X', ms=7, markeredgewidth=2, markeredgecolor='black', label=mname, clip_on=False)[0]
                    if '3' in plotMode or '4' in plotMode:
                        if iidx > 0:
                            xy = line_coors.loc[mname, col][iidx-1]
                            if not np.isnan(xy[1]):
                                ax.plot([xy[0], X_], [xy[1], Y_], color=linecolor, linewidth=2, label=mname)
                            else:
                                if not np.isnan(Y_):
                                    ax.plot([xy[0], X_], [0, Y_], color=linecolor, linewidth=2, linestyle='dashed', label=mname)
                                else:
                                    ax.plot([xy[0], X_], [0, 0], color=linecolor, linewidth=2, linestyle='dotted', label=mname)
        ...
        if idx == 0:
            ...
            if 'mice' in legend or 'all' in legend:
                AS.legend_mice(ax, list(chain.from_iterable(mouse_names)), symbol='o', loc='upper center')
        sleepy.box_off(ax)
    plt.show()
...
def avg_waveform(ppath, recordings, istate, win=[0.5,0.5], mode='pwaves', tstart=0, 
                 tend=-1, ma_thr=20, ma_state=3, flatten_tnrem=False, mouse_avg='mouse', 
                 plaser=True, post_stim=0.1, lsr_iso=0, pload=False, psave=False, p_iso=0, 
                 pcluster=0, clus_event='waves', wform_std=True, ylim=[]):
    """
    Calculate average LFP waveforms for P-waves and laser pulses in any brain state
    @Params
    ppath - base folder
    recordings - list of recordings
    istate - brain state(s) to analyze
    win - time window (s) to collect data relative to given events
    mode - event-triggered LFP(s) to plot
           'pwaves' - plot LFP surrounding spontaneous P-waves
                       * if plaser=True, also plot LFP of laser-triggered P-waves
           'lsr' - plot LFP surrounding successful and failed laser pulses
    tstart, tend - time (s) into recording to start and stop collecting data
    ma_thr, ma_state - max duration and brain state for microarousals
    flatten_tnrem - brain state for transition sleep
    mouse_avg - method for data averaging; by 'mouse', 'rec'[ording], or 'trial'
    ...
    psave - optional string specifying a filename to save the data (if False, data is not saved)
    p_iso, pcluster, clus_event - see SOMETHING ELSE
    wform_std - if True, plot average LFP +/- st. deviation; if False, plot LFP mean only
    ...
    states = {'total':'total', 1:'REM', 2:'Wake', 3:'NREM', 4:'tNREM', 5:'failed-tNREM', 6:'Microarousals'}
    
    # load LFP data
    if pload:
        data = AS.load_surround_files(ppath, pload=pload, istate=istate, plaser=plaser, 
                                      null=False, signal_type='LFP')
        if len(data) > 0:
            if plaser:
    ...
    if not pload:
        if plaser:
            data = get_lsr_surround(ppath, recordings, istate=istate, win=win, signal_type='LFP', tstart=tstart, 
                                    tend=tend, ma_thr=ma_thr, ma_state=ma_state, flatten_tnrem=flatten_tnrem, 
                                    null=False, post_stim=post_stim, lsr_iso=lsr_iso, psave=psave, 
                                    p_iso=p_iso, pcluster=pcluster, clus_event='waves')
            lsr_pwaves, spon_pwaves, success_lsr, fail_lsr, null_pts, data_shape = data
        elif not plaser:
            data = get_surround(ppath, recordings, istate=istate, win=win, signal_type='LFP', tstart=tstart, 
                                tend=tend, ma_thr=ma_thr, ma_state=ma_state, flatten_tnrem=flatten_tnrem, 
                                null=False, psave=psave, p_iso=p_iso, pcluster=pcluster, clus_event=clus_event)
            p_signal, null_signal, data_shape = data
                                                                                                                      
    x = np.linspace(-np.abs(win[0]), win[1], data_shape[0])
    ...
    if plaser:
        for s in istate:
            ...
            elif mouse_avg == 'rec' or mouse_avg == 'mouse':
...
def avg_SP(ppath, recordings, istate, win=[5,5], mode='pwaves', recalc_highres=False, 
           ...
           nsr_seg=2, perc_overlap=0.95, fmax=30, ma_thr=20, ma_state=3, flatten_tnrem=False,
    ...
    states = {'total':'total', 1:'REM', 2:'Wake', 3:'NREM', 4:'tNREM', 
              5:'failed-tNREM', 6:'Microarousals'}
    ...
    if not pload:
        if plaser:
            data = get_lsr_surround(ppath, recordings, istate=istate, win=win, signal_type=signal_type, recalc_highres=recalc_highres,
                                    tstart=tstart, tend=tend, ma_thr=ma_thr, ma_state=ma_state, flatten_tnrem=flatten_tnrem,
...
def avg_band_power(ppath, recordings, istate, bands, band_labels=[], band_colors=[], win=[5,5], 
                   ...
                   flatten_tnrem=False, mouse_avg='mouse', p_iso=0, pcluster=0, clus_event='waves', 
    ...
    if not pload:
        if plaser:
            data = get_lsr_surround(ppath, recordings, istate=istate, win=win, signal_type=signal_type, 
                                    ...
                                    ma_state=ma_state, flatten_tnrem=flatten_tnrem, nsr_seg=nsr_seg, perc_overlap=perc_overlap, 
            ...
            p_signal, null_signal, data_shape = get_surround(ppath, recordings, istate=istate, win=win, signal_type=signal_type, 
                                                             recalc_highres=recalc_highres, tstart=tstart, tend=tend, ma_thr=ma_thr,
                                                             ma_state=ma_state, flatten_tnrem=flatten_tnrem, nsr_seg=nsr_seg, 
...
def sp_profiles(ppath, recordings, spon_win=[1,1], frange=[6,15], recalc_highres=False, tstart=0, tend=-1, ma_thr=20, ma_state=3, 
                flatten_tnrem=False, nsr_seg=2, perc_overlap=0.95, pnorm=0, psmooth=0, pcalc=0, null=True, null_win=[-0.5,0.5], null_match='spon',
...
def lsr_hilbert(ppath, recordings, istate, bp_filt, stat='perc', mode='pwaves',
                ...
                flatten_tnrem=False, post_stim=0.1, bins=9, pload=False, psave=False):
    """
    ...
    if mouse_avg == 'mouse' or mouse_avg == 'rec':
    ...
    if stat == 'count':
        if mouse_avg == 'rec' or mouse_avg == 'mouse':
...
def lsr_state_success(stats_df, istate, jstate=[], plotMode='0'):
    ...
    sns.barplot(x='State', y='Perc', data=df, ci=68, palette={'REM':'cyan','Wake':'darkviolet','NREM':'darkgray', 
                                                              'tNREM':'darkblue','sham REM':'cadetblue','sham wake':'rebeccapurple',
                                                              'sham NREM':'dimgray', 'sham tNREM':'darkgreen'})
...
def lsr_prev_theta_success(ppath, recordings, win=[-2,0], theta_band=[6,15], mode='power', 
                           post_stim=0.1, ma_thr=20, ma_state=3, flatten_tnrem=False, 
    ...
    if not pload:
        data_lsr = get_lsr_surround(ppath, recordings, istate=istate, win=win, signal_type=signal_type,
                            recalc_highres=False, ma_thr=ma_thr, ma_state=ma_state, flatten_tnrem=flatten_tnrem, 
